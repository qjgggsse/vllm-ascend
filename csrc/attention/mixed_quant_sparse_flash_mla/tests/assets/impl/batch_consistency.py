...
"""Batch-consistency input randomization for MixedQuantSparseFlashMla TTK cases."""
...
class NumpyBatchRandomContext:
    ...
    def __init__(self, q, ori_kv, cmp_kv, kwargs):
        ...
        if self.layout_kv == "TND" and any(
            ...
            raise ValueError(
                "MQSMLA TND KV relations cannot map a strided logical B slice"
            )
    ...
    def parse_slice(value, extent, label):
        ...
        if step <= 0 or start < 0 or start >= stop or stop > extent:
            raise ValueError(
                f"MQSMLA {label} slice must be in-range, non-empty and have a positive step: {value!r}"
            )
        ...
        for batch_slice, sequence_slice, _seed in self.relations:
            ...
            if self.layout_q == "BSND":
                ...
                if batch_step != 1 and len(batch_indices) > 1:
                    raise ValueError(
                        "MQSMLA TND logical B relation requires a contiguous B slice"
                    )
                physical_stop = (
    ...
    def register_prefix(self, prefix, source):
        ...
        for batch_slice, _seed in self.batch_relations:
            ...
            if step != 1 and len(batch_indices) > 1:
                raise ValueError(
                    "MQSMLA TND KV relations require contiguous logical B slices"
                )
            physical_stop = (
