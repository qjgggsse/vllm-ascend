...
static_assert(TQ4_SCALE_UB_F32 * sizeof(float) >= 1024U,
              "TQ4 fp32 area must start after the half staging area");
...
template <typename QSFAT> __aicore__ inline void QSFAVectorService<QSFAT>::InitBuffers(TPipe *pipe)
{
    ...
    Duplicate(tq4ScaleBuf_.Get<T>()[TQ4_SCALE_UB_F32], static_cast<T>(1.0),
              TQ4_SCALE_SLOT_STRIDE);
    ...
    const GlobalTensor<K_ROPE_T> &keyRopeGm, const GlobalTensor<KV_T> &keyGm, const GlobalTensor<int32_t> &blkTableGm)
{
    ...
    this->tq4ScaleGm_.SetGlobalBuffer(
        reinterpret_cast<__gm__ uint16_t *>(kvValidSizeGm.GetPhyAddr(0)));
}
    ...
    if constexpr (TEMPLATE_MODE == V_TEMPLATE) {
        ...
        if (constInfo.keyQuantMode == QUANT_MODE::TQ4) {
            ...
            DataCopyPad(tq4ScaleBuf_.Get<uint16_t>(),
                        tq4ScaleGm_[TQ4_SCALE_HALF_BASE +
                             info.loop % MERGE_CACHE_GM_BUF_NUM * TQ4_SCALE_SLOT_STRIDE],
                                                                   ...
                                                                   const RunInfo &runInfo)
{
    ...
    if (constInfo.keyQuantMode == QUANT_MODE::TQ4) {
        ...
        {
            uint32_t qsfaScaleByteOff = constInfo.headDim / 2 +
            ...
            uint32_t qsfaSlotRowBlk = QSFAAlign(static_cast<uint32_t>(tilingData->baseParams.dSizeVInput),
                                                static_cast<uint32_t>(BYTE_BLOCK)) / BYTE_BLOCK;
            ...
            LocalTensor<half> qsfaSrcHalf =
                srcTensor.template ReinterpretCast<half>()[qsfaScaleByteOff / 2];
            ...
            DataCopyPad(tq4ScaleGm_[TQ4_SCALE_HALF_BASE +
                             runInfo.loop % MERGE_CACHE_GM_BUF_NUM * TQ4_SCALE_SLOT_STRIDE +
...
__aicore__ inline void QSFAVectorService<QSFAT>::MergeKv(const RunInfo &runInfo)
{
    ...
    if (unlikely(qsfaS2GmStartOffset + qsfaMte2Size < qsfaS2GmLimit)) {
        ...
        if (constInfo.keyQuantMode == QUANT_MODE::TQ4) {
            ...
            dataCopyParams.srcStride = 0;
            dataCopyParams.dstStride = 0;
            for (int64_t s2GmOffset = qsfaS2GmStartOffset + qsfaMte2Size;
                 s2GmOffset < qsfaS2GmLimit; s2GmOffset++) {
