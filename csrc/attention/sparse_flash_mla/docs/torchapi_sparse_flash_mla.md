...
| cmp_ratio | int32 | 可选 | `cmp_kv`相对于压缩前kv的压缩倍率，取值范围为1-128，默认值为1。 | int32 | - | - |
...
| cmp_ratio | int32 | 可选 | 压缩倍率，取值范围为1-128，默认值为1。 | int32 | - | - |
...
| :--- | :--- | :--- |
| SWA | 仅传入`ori_kv`。 | 不传`cmp_kv`、`cmp_sparse_indices`和`cmp_block_table`；`cmp_topk=0`、`cmp_mask_mode=0`。 |
