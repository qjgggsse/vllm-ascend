...
constexpr uint32_t PACKED_BYTES = 256;
constexpr uint32_t SCALE_BYTES = 2;
constexpr uint32_t N_CENT = 16;
constexpr size_t WORKSPACE_SIZE = 32;
constexpr char DATA_DIR[] = "./turbo_quant_data";
} // namespace

class turbo_quant_test : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "turbo_quant_test SetUp\n" << std::endl; }
    static void TearDownTestCase() { std::cout << "turbo_quant_test TearDown\n" << std::endl; }
};
...
static void RunCase(uint32_t numTokens, uint32_t tokensPerCore, uint32_t tokensPerBatch, const char* dist = "gauss_lat")
{
    ...
    system("cp -rf "
           "../../../../quant/turbo_quant/tests/ut/op_kernel/"
           "turbo_quant_data ./");
    system("chmod -R 755 ./turbo_quant_data/");
    ...
    size_t yByteSize = static_cast<size_t>(numTokens) * PACKED_BYTES;
    size_t scaleByteSize = static_cast<size_t>(numTokens) * SCALE_BYTES;
    ...
    uint8_t* y = (uint8_t*)AscendC::GmAlloc(yByteSize == 0 ? 1 : yByteSize);
    uint8_t* scale = (uint8_t*)AscendC::GmAlloc(scaleByteSize == 0 ? 1 : scaleByteSize);
    uint8_t* workspace = (uint8_t*)AscendC::GmAlloc(WORKSPACE_SIZE);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(sizeof(TurboQuantTilingData));
    ...
    TurboQuantTilingData* tilingData = reinterpret_cast<TurboQuantTilingData*>(tiling);
    tilingData->numTokens = numTokens;
    tilingData->tokensPerCore = tokensPerCore;
    tilingData->headDim = HEAD_DIM;
    tilingData->packedBytes = PACKED_BYTES;
    tilingData->tokensPerBatch = tokensPerBatch;

    ICPU_RUN_KF(turbo_quant, blockDim, latent, centroids, y, scale, workspace, (uint8_t*)(tilingData));

    WriteFile(std::string(DATA_DIR) + "/output_y.bin", y, yByteSize == 0 ? 1 : yByteSize);
    WriteFile(std::string(DATA_DIR) + "/output_scale.bin", scale, scaleByteSize == 0 ? 1 : scaleByteSize);
...
TEST_F(turbo_quant_test, single_token_single_core) { RunCase(1, 1, 1); }

// One token per core: exercises the tokStart_ == tokEnd_ - 1 path on every core.
TEST_F(turbo_quant_test, one_token_per_core) { RunCase(8, 1, 1); }

// Ragged split: the last core gets fewer tokens than tokensPerCore, so tokEnd_ is clamped.
TEST_F(turbo_quant_test, ragged_last_core) { RunCase(33, 8, 1); }

TEST_F(turbo_quant_test, multi_token_per_core) { RunCase(64, 16, 1); }

// Batched: every core runs whole batches of 4.
TEST_F(turbo_quant_test, batched_exact) { RunCase(64, 16, 4); }

// Two kinds of tail at once: 8 tokens per core in batches of 3 (3+3+2), and the last core owns 1 token.
TEST_F(turbo_quant_test, batched_ragged_tail) { RunCase(33, 8, 3); }

// Batch at the compile-time maximum.
TEST_F(turbo_quant_test, batched_max) { RunCase(24, 12, 12); }

// Batch larger than the tokens a core owns: the count clamps on the very first batch.
TEST_F(turbo_quant_test, batch_larger_than_tokens) { RunCase(5, 5, 12); }

// Empty tensor: tiling still hands out one block, the kernel must write nothing and not fault.
TEST_F(turbo_quant_test, empty_tensor) { RunCase(0, 1, 1); }
...
TEST_F(turbo_quant_test, uniform_wide_range) { RunCase(33, 8, 8, "uniform"); }

TEST_F(turbo_quant_test, gaussian_shifted_wide) { RunCase(33, 8, 8, "gauss_wide"); }
...
// The fp16 scale carries the NaN/INF through.
TEST_F(turbo_quant_test, input_nan) { RunCase(9, 4, 4, "nan"); }

TEST_F(turbo_quant_test, input_pos_inf) { RunCase(9, 4, 4, "posinf"); }

TEST_F(turbo_quant_test, input_neg_inf) { RunCase(9, 4, 4, "neginf"); }

TEST_F(turbo_quant_test, input_mixed_non_finite) { RunCase(9, 4, 4, "mixinf"); }
