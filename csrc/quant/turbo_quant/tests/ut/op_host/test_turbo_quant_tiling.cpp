...
/*!
 * \file test_turbo_quant_tiling.cpp
 * \brief
 */
...
#include "quant/turbo_quant/op_host/turbo_quant_tiling.h"
...
constexpr size_t IDX_PACKED_BYTES = 3;
constexpr size_t IDX_TOKENS_PER_BATCH = 4;
} // namespace

class TurboQuantTiling : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "TurboQuantTiling SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "TurboQuantTiling TearDown" << std::endl; }
};
...
static TilingResult RunTiling(int64_t numTokens, int64_t headDim, int64_t centCount,
                              ge::DataType latentDtype = ge::DT_FLOAT, ge::DataType centroidsDtype = ge::DT_FLOAT,
                              size_t centRank = 1)
{
    ...
    optiling::TurboQuantCompileInfo compile_info;

    std::string op_type("TurboQuant");
    ...
    gert::StorageShape yShape = {{numTokens, headDim / 2}, {numTokens, headDim / 2}};
    gert::StorageShape scaleShape = {{numTokens}, {numTokens}};
    ...
    auto holder = gert::TilingContextFaker()
                      ...
                      .NodeInputTd(0, latentDtype, ge::FORMAT_ND, ge::FORMAT_ND)
                      .NodeInputTd(1, centroidsDtype, ge::FORMAT_ND, ge::FORMAT_ND)
...
static void ExpectValidSplit(const TilingResult& r, int64_t numTokens)
{
    ...
    EXPECT_EQ(r.fields[IDX_PACKED_BYTES], 256U);
    ...
    EXPECT_LE(r.fields[IDX_TOKENS_PER_BATCH], static_cast<uint32_t>(optiling::TQ_MAX_TOKENS_PER_BATCH));
    EXPECT_LE(r.fields[IDX_TOKENS_PER_BATCH], r.fields[IDX_TOKENS_PER_CORE]);
}

TEST_F(TurboQuantTiling, single_token)
{
...
TEST_F(TurboQuantTiling, fewer_tokens_than_cores)
{
...
TEST_F(TurboQuantTiling, prefill_saturates_batch)
{
    TilingResult r = RunTiling(2048, 512, 16);
    ExpectValidSplit(r, 2048);
    EXPECT_EQ(r.fields[IDX_TOKENS_PER_BATCH], static_cast<uint32_t>(optiling::TQ_MAX_TOKENS_PER_BATCH));
}

// in between, the batch tracks what a core actually owns
TEST_F(TurboQuantTiling, batch_tracks_tokens_per_core)
{
...
TEST_F(TurboQuantTiling, latent_float16_is_unsupported)
{
    TilingResult r = RunTiling(128, 512, 16, ge::DT_FLOAT16);
    EXPECT_EQ(r.status, ge::GRAPH_FAILED);
}

TEST_F(TurboQuantTiling, latent_bfloat16_is_unsupported)
{
    TilingResult r = RunTiling(128, 512, 16, ge::DT_BF16);
    EXPECT_EQ(r.status, ge::GRAPH_FAILED);
}

TEST_F(TurboQuantTiling, ragged_token_count) { ExpectValidSplit(RunTiling(4097, 512, 16), 4097); }

TEST_F(TurboQuantTiling, large_batch) { ExpectValidSplit(RunTiling(65536, 512, 16), 65536); }

TEST_F(TurboQuantTiling, zero_tokens)
{
...
TEST_F(TurboQuantTiling, unsupported_head_dim) { EXPECT_EQ(RunTiling(16, 256, 16).status, ge::GRAPH_FAILED); }

TEST_F(TurboQuantTiling, wrong_centroid_count) { EXPECT_EQ(RunTiling(16, 512, 8).status, ge::GRAPH_FAILED); }

TEST_F(TurboQuantTiling, non_float_centroids)
{
    EXPECT_EQ(RunTiling(16, 512, 16, ge::DT_FLOAT, ge::DT_FLOAT16).status, ge::GRAPH_FAILED);
}

TEST_F(TurboQuantTiling, non_1d_centroids_are_unsupported)
{
    EXPECT_EQ(RunTiling(16, 512, 16, ge::DT_FLOAT, ge::DT_FLOAT, 2).status, ge::GRAPH_FAILED);
}

TEST_F(TurboQuantTiling, token_count_overflows_tiling_field)
{
    const int64_t tooManyTokens = static_cast<int64_t>(std::numeric_limits<uint32_t>::max()) + 1;
    EXPECT_EQ(RunTiling(tooManyTokens, 512, 16).status, ge::GRAPH_FAILED);
}
